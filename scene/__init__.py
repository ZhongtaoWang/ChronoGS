...
class Scene:
    ...
    def __init__(self, args, gaussians, load_iteration=None, shuffle=True, resolution_scales=[1.0], ply_path=None, logger=None):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse","split.txt")):
            with open(os.path.join(args.source_path, "sparse","split.txt"), 'r') as file:
                time_stamp_map = {line.strip().split(" ")[0].split(".")[0]: int(line.strip().split(" ")[1]) for line in file}
            self.time_counts = len(list(set(time_stamp_map.values())))
        else:
            time_stamp_map = None
            self.time_counts = None

        print(f"self.time_counts: {self.time_counts}")

        self.gaussians.set_global_feat(self.time_counts)
        
        if hasattr(self.gaussians, 'times'):
            self.gaussians.set_times(self.time_counts)
        else:
            self.gaussians.times = self.time_counts
        ...
        for resolution_scale in self.resolution_scales:
            ...
            self.train_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.train_cameras, resolution_scale, args, self.background, time_stamp_map)
            print("Loading Test Cameras")
            self.test_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.test_cameras, resolution_scale, args, self.background, time_stamp_map)
        ...
        if self.loaded_iter:
            ...
            self.gaussians.create_from_pcd(pcd, self.cameras_extent, logger, self.train_cameras, self.resolution_scales, times = self.time_counts)
