...
def readColmapSceneInfo(path, images, eval,  llffhold=8, warmup_ply_path=None):
    ...
    if eval:
        if os.path.exists(os.path.join(path, "sparse","0" , "test.txt")):
            with open(os.path.join(path, "sparse","0" , "test.txt"), 'r') as file:
                test_cam_names_list = [os.path.basename(line.strip()).split(".")[0] for line in file]
            ...
            print(f'test_cam_names_list: {test_cam_names_list}')
        else:
    ...
    if warmup_ply_path is not None:
        ...
        try:
            xyz, rgb, _, point3D_id_map = read_points3D_binary(bin_path)
        except:
            xyz, rgb, _, point3D_id_map = read_points3D_text(txt_path)
