...
def loadCam(args, id, cam_info, resolution_scale, background , time_stamp = None):
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  uid=id, data_device=args.data_device, time_stamp=time_stamp)

def cameraList_from_camInfos(cam_infos, resolution_scale, args, background , time_stamp_map = None):
    camera_list = []
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        if time_stamp_map is not None:
            futures = [executor.submit(loadCam, args, cam_id, c, resolution_scale, background, time_stamp_map[c.image_name]) for cam_id, c in enumerate(cam_infos)]
        else:
