...
def render_set(base_model, model_path, name, iteration, views, gaussians, pipe, background, render_mode, ape_code):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if view.time_stamp is not None:
            if view.time_stamp not in time_stamp_views:
                time_stamp_views[view.time_stamp] = []
            time_stamp_views[view.time_stamp].append(view)
    ...
    print(f"len of time_stamp_views: {len(time_stamp_views)}")
    for time_stamp, views in time_stamp_views.items():
        per_view_dict_time_stamp = {}
        render_path = os.path.join(model_path, name, "ours_{}_{}".format(iteration, time_stamp), "renders")
        makedirs(render_path, exist_ok=True)
        gts_path = os.path.join(model_path, name, "ours_{}_{}".format(iteration, time_stamp), "gt")
        makedirs(gts_path, exist_ok=True)
        ...
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
            ...
            rendering = torch.clamp(render_pkg["render"], 0.0, 1.0)
            visible_count = render_pkg["visibility_filter"].sum()  
            per_view_dict_time_stamp['{0:05d}'.format(idx)+".png"] = visible_count.item()

            gt = view.original_image[0:3, :, :]
            torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
            torchvision.utils.save_image(gt, os.path.join(gts_path, '{0:05d}'.format(idx) + ".png"))

        with open(os.path.join(model_path, name, "ours_{}_{}".format(iteration, time_stamp), "per_view_count.json"), 'w') as fp:
            json.dump(per_view_dict_time_stamp, fp, indent=True) 

    
    for time_stamp in range(len(time_stamp_views)):
        per_view_dict_time_stamp = {}
        render_path = os.path.join(model_path, name, f"ours_alltime_{time_stamp}_{iteration}", "renders")
        makedirs(render_path, exist_ok=True)
        gts_path = os.path.join(model_path, name, f"ours_alltime_{time_stamp}_{iteration}", "gt")
        makedirs(gts_path, exist_ok=True)

        for idx, view in enumerate(tqdm(raw_views, desc="Rendering progress")):

            torch.cuda.synchronize(); t0 = time.time()
            view.time_stamp = time_stamp
            if ape_code==-1:
                render_pkg = getattr(modules, get_render_func(base_model))(view, gaussians, pipe, background, iteration, render_mode)
            else:
                render_pkg = getattr(modules, get_render_func(base_model))(view, gaussians, pipe, background, iteration, render_mode, ape_code)
        ...
        with open(os.path.join(model_path, name, f"ours_alltime_{time_stamp}_{iteration}", "per_view_count.json"), 'w') as fp:
            json.dump(per_view_dict_time_stamp, fp, indent=True) 
    # save the time_stamp_views to a json file
    t = np.array(t_list[5:])
    fps = 1.0 / t.mean()
