...
def prefilter_voxel(viewpoint_camera, pc, pipe, bg_color):
    ...
    # radii shape is [C, N, 2], squeeze camera dimension and check if any radius > 0
    radii_squeezed = radii.squeeze(0)  # [N, 2]
    visible_mask[pc._anchor_mask] = (radii_squeezed > 0).any(dim=-1)
