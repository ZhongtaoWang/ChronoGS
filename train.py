...
# python train.py --config config/chronogs.yaml -s /data6/wangzhongtao/TimeGS/data_colmap_transformed/jiayuan_merge_transformed -m /data6/wangzhongtao/TimeGS/chronogs_result/jiayuan
...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        render_pkg = getattr(modules, get_render_func(dataset.base_model))(viewpoint_cam, gaussians, pipe, scene.background, iteration, dataset.render_mode, -1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[ i  for i in range(30000, 40000 + 1, 1000)])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[ i  for i in range(30000, 40000 + 1, 1000)])
    ...
    if args.model_path is None:
        ...
        print(f'using model path: {lp.model_path}')
    
    if args.source_path is not None:
        lp.source_path = args.source_path
        print(f'using source path: {lp.source_path}')
